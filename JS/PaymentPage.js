...
if (redirectParam === 'true') {
    // Set the redirect parameter to false and reload the page
  urlParams.set('redirect', 'false');
  // Retrieve the array of invoice IDs from local storage
var selectedInvoiceIds = JSON.parse(localStorage.getItem('selectedInvoiceIds'));

// Update the data based on the selected invoice IDs
for (var i = 0; i < selectedInvoiceIds.length; i++) {
  var invoiceId = selectedInvoiceIds[i];

  // Find the corresponding invoice in the data array
  var invoiceToUpdate = data.find(function(invoice) {
...
// Clear the stored invoice IDs from local storage
localStorage.removeItem('selectedInvoiceIds');
